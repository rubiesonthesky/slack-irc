...
Bot.prototype.parseText = function(text) {
  return text
    ...
    // getBotByID
    .replace(/<@(U\w+)\|?(\w+)?>/g, function(match, botId, readable) {
      return readable || '@' + this.slack.getBotByID(botId).name;
    }.bind(this))
...
Bot.prototype.sendToIRC = function(message) {
  ...
  if (ircChannel) {
    if (this.slack.getBotByID()) {
      var user = this.slack.getBotByID(message.user)
    }
    else {
